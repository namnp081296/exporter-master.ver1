...
#arr_port+=( ["php_fpm"]=8080 ["mongodb"]=9001 ["merger"]=39000 )
...
# Will get the value of VERSION_ID
...
PROGNAME=${expter}_exporter
PROG=/usr/local/bin/${expter}_exporter
USER=root
LOGFILE=/var/log/${expter}_exporter.log
LOCKFILE=/var/run/${expter}_exporter.pid


start() {
    echo -n "Starting $PROGNAME: "
    cd /usr/local/bin/
    daemon --user $USER --pidfile="$LOCKFILE" "$PROG `/bin/bash ~/exporter/yaml_handler/parse_yml.sh ${expter}`$new_port &>$LOGFILE &"
...
start() {
    echo -n "Starting $PROGNAME: "
    cd /usr/local/bin/
    daemon --user $USER --pidfile="$LOCKFILE" "$PROG `/bin/bash ~/exporter/yaml_handler/parse_yml.sh ${expter}`$newport_rand &>$LOGFILE &"
    echo $(pidofproc $PROGNAME) >$LOCKFILE
    echo
}
...
ADDTEXT
 ...
 chmod 755 /etc/init.d/*_exporter
 chkconfig --add ${expter}_exporter
 chkconfig ${expter}_exporter on
 service ${expter}_exporter start
 service ${expter}_exporter status
